...
function search(options, graph, map) {
    ...
    // priority queue key
    let order_val = options.queue == 'lifo' ? -1 : +1;
    ...
    while (s.frontier.length > 0) {
        ...
        s.neighbors.forEach(function (next) {
            ...
            if (!map[next].visited
                ...
                map[next].visit_order = order_val*visit_order++;
...
var SearchLoop = /** @class */ (function () {
    function SearchLoop(parent_selector, diagrams) {
        var _this = this;
    ...
    SearchLoop.prototype.loop = function () {
...
function makeDiagram9() {
    ...
    var options_c = new SearchOptions(options_a.starts, options_a.exit_now, function (id, node) { node.h = manhattan_heuristic(graph, exit.id, id); return node.cost_so_far + node.h; });
    ...
    new SearchLoop("#diagram-astar-3", [diagram_c]);


    var o = new SearchOptions(options_a.starts, options_a.exit_now, function (id, node) { node.h = manhattan_heuristic(graph, exit.id, id); return node.cost_so_far + node.h; });
    o.queue = 'lifo';
    var d = new Diagram("#diagram-astar-4", graph, o, layout, [
        [BaseLayer],
        [GraphEditorLayer, [1, 5, Infinity]],
        [ReconstructedPathLayer, function () { return exit.id; }],
        [NumericLabelLayer, 'sort_key'],
...
*/
    ...
    let starts = [graph.to_id(0, 12)];
    let exit_now = function (ss) { return ss.current == exit.id; };
    let key = function (id, node) { node.h = manhattan_heuristic(graph, exit.id, id); return node.cost_so_far + node.h; };
    ...
    function make(selector, o) {
        ...
        var d = new Diagram(selector, graph, o, layout, layers);
        ds.push(d);
    }

    var o = new SearchOptions(starts, exit_now, key);
    o.queue = 'lifo';
    o.shuffle = false;
    make("#diagram-astar-3", o);

    var o = new SearchOptions(starts, exit_now, key);
    o.queue = 'lifo';
    o.shuffle = true;
    make("#diagram-astar-4", o);

    var o = new SearchOptions(starts, exit_now, key);
    o.queue = 'fifo';
    o.shuffle = false;
    make("#diagram-astar-5", o);

    var o = new SearchOptions(starts, exit_now, key);
    o.queue = 'fifo';
    o.shuffle = true;
    make("#diagram-astar-6", o);

    new SearchLoop("#search-loop", ds);
